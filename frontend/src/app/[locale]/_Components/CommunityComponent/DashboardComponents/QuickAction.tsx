...
import React from "react";
import {
  CircleAlert, Bell, MessageSquare, Users, Eye, TrendingUp, FileText,
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Link } from "@/i18n/navigation";
...
<Link href={`/student/projects/${projectId}?tab=supervision`}>
  <Button className="gap-2 text-foreground hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 h-9 px-4 py-2 w-full justify-start mb-2 cursor-pointer bg-background">
    <Bell className="mr-2 size-4" />
    Review Requests
  </Button>
      ...
      {/* ✅ Upload Document → بيودي لـ Deliverables tab بتاعة المشروع الحالي، مش زرار فاضي */}
      {variant === "student" && hasProject && projectId && (
        <Link href={`/student/projects/${projectId}?tab=deliverables`}>
          <Button className="cursor-pointer gap-2 text-foreground hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 h-9 px-4 py-2 w-full justify-start bg-background">
            <FileText className="mr-2 size-4" />
            Upload Deliverable
          </Button>
      ...
      {/* ✅ "Message Supervisor" استبدلتها بـ "Schedule Meeting" — موجودة فعلياً في الـ API */}
      {variant === "student" && hasProject && projectId && (
        <Link href={`/student/projects/${projectId}?tab=meetings`}>
          <Button className="cursor-pointer gap-2 text-foreground hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 h-9 px-4 py-2 w-full justify-start bg-background">
            <MessageSquare className="mr-2 size-4" />
            Schedule Meeting
          </Button>
      ...
      {/* ✅ "View Analytics" مفيهاش API أصلاً، استبدلتها بـ "View Feedback" اللي حقيقية */}
      {variant === "supervisor" && (
        <Link href="/supervisor/projects">
          <Button className="cursor-pointer gap-2 text-foreground hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50 h-9 px-4 py-2 w-full justify-start bg-background">
            <TrendingUp className="mr-2 size-4" />
