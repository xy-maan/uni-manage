...
// import { GetSupervisorDataAction } from "@/Actions/getSupervisorData.action";
...
// }) {
//   const { payload: dataStudent } = await GetStudentDataAction();
//   const { payload: dataSupervisor } = await GetSupervisorDataAction();
//   return (
...
//                             value={66}
//                             id="progress-upload"
//                             className="bg-muted  [&_[dataStudent-slot=progress-indicator]]:!bg-primary [&_[dataStudent-slot=progress-indicator]]:!bg-none"
//                           />
//                         </Field>
//                       </TooltipTrigger>
...
//   );
// }
// app/[locale]/[role]/profile/page.tsx (للـ supervisor)
import { GetStudentDataAction } from "@/Actions/getStudentData.action";
import { GetSupervisorDataAction } from "@/Actions/getSupervisorData.action";
import EditProfileBtn from "@/app/[locale]/_Components/ProfileComponents/EditProfileBtn/EditProfileBtn";
import { Badge } from "@/components/ui/badge";
...
import { Metadata } from "next";
import {
  Award, Briefcase, ExternalLink, GraduationCap, Mail, Phone, Pin, Plus, Star, Users
} from "lucide-react";
import { Link } from "@/i18n/navigation";
import StudentProfile from "@/app/[locale]/_Components/StudentProfile/StudentProfile";
...
export default async function ProfilePage({
  ...
  const { payload: dataSupervisor } = role === "supervisor" 
    ? await GetSupervisorDataAction() 
    : { payload: null };
    
  const { payload: dataStudent } = role === "student" 
    ? await GetStudentDataAction() 
    : { payload: null };
  return (
    <div className="container mx-auto px-4 lg:px-8 py-8">


      {role==="supervisor"&&  <div className="container mx-auto px-4 lg:px-8 py-8">
          {/* Header Card */}
          <div className="bg-card text-card-foreground flex flex-col gap-6 rounded-xl border mb-8">
            <div className="pb-6 p-8">
              <div className="flex flex-col md:flex-row gap-6 items-start">
                <div className="rounded-full h-32 w-32 border-4 border-primary/20 overflow-hidden text-white">
                  <span className="bg-linear-to-br size-full from-primary to-secondary rounded-full text-3xl flex items-center justify-center">
                    {dataSupervisor?.user?.first_name?.charAt(0).toUpperCase() || "U"}
                  </span>
                </div>
                ...
                <div className="content flex-1 w-full">
                  <div className="flex-col flex-1">
                    <div className="w-full flex items-center justify-between lg:mb-2 mb-4 lg:items-start">
                      <h3 className="lg:mb-2 text-[16px]">
                        {dataSupervisor.user.first_name} {dataSupervisor.user.last_name}
                      </h3>
                      <div className="flex items-center gap-1.5">
                        <Badge className="bg-success/10 text-success border-success/30 text-sm px-2.5 py-1 flex items-center gap-1.5 font-medium">
                          <Award className="size-4" />
                          {dataSupervisor.title_display}
                        </Badge>
                        <EditProfileBtn />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      <Badge className="bg-primary/10 text-primary">
                        <GraduationCap className="size-3 mr-1" />
                        {dataSupervisor.user.role}
                      </Badge>
                      <Badge className="bg-secondary/10 text-secondary">
                        {dataSupervisor.department?.name}
                      </Badge>
                      <Badge className="bg-warning/10 text-warning">
                        <Users className="size-3 mr-1" />
                        Max Capacity: {dataSupervisor.max_team_capacity}
                      </Badge>
                    </div>
    
                    <p className="text-muted-foreground mb-4">
                      {dataSupervisor.title_display} specializing in{" "}
                      {dataSupervisor.expertise?.map((e: any) => e.name).join(", ")}.
                    </p>
    
                    <div className="grid md:grid-cols-3 gap-3 mb-4">
                      <div className="flex items-center gap-2 text-sm hover:text-primary transition-colors cursor-pointer">
                        <Mail className="size-4 text-muted-foreground" />
                        {dataSupervisor.user.email}
                      </div>
                      {dataSupervisor.scholar_url && (
                        <Link
                          href={dataSupervisor.scholar_url}
                          target="_blank"
                      ...
                      {dataSupervisor.linkedin_url && (
                        <Link
                          href={dataSupervisor.linkedin_url}
                          target="_blank"
                          className="flex items-center gap-2 text-sm hover:text-primary transition-colors cursor-pointer"
                        >
                          <ExternalLink className="size-4 text-muted-foreground" />
          ...
          <Tabs defaultValue="overview" className="w-full space-y-6">
            ...
            <TabsContent value="overview">
              <div className="grid md:grid-cols-2 gap-6">
                <Card className="border hover:border-primary/50 transition-all">
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between">
                      <span className="text-md font-normal">Expertise</span>
                      <Button className="bg-transparent hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 size-9">
                        <Plus className="size-4" />
                      </Button>
                    </CardTitle>
                  ...
                  <CardContent className="text-sm text-muted-foreground flex flex-wrap gap-2">
                    {dataSupervisor.expertise?.map((exp: any) => (
                      <Badge key={exp.id} className="bg-secondary text-secondary-foreground capitalize">
                        {exp.name}
                      </Badge>
                    ))}
                    {dataSupervisor.expertise?.length === 0 && (
                  ...
                  <CardContent className="text-sm text-muted-foreground grid grid-cols-2 gap-4">
                    <div className="text-center p-4 rounded-lg bg-primary/5 hover:bg-primary/10 transition-colors">
                      <span className="text-2xl font-bold text-primary">
                        {dataSupervisor.supervised_projects_count ?? 0}
                      </span>
                      <h3 className="text-sm text-muted-foreground">Supervised Projects</h3>
                    </div>
                    <div className="text-center p-4 rounded-lg bg-secondary/5 hover:bg-secondary/10 transition-colors">
                      <span className="text-2xl font-bold text-secondary">
                        {dataSupervisor.max_team_capacity}
                      </span>
                      <h3 className="text-sm text-muted-foreground">Max Capacity</h3>
                    </div>
                    <div className="text-center p-4 rounded-lg bg-success/5 hover:bg-success/10 transition-colors">
                      <span className="text-2xl font-bold text-success">
                        {dataSupervisor.expertise?.length ?? 0}
                      </span>
                      <h3 className="text-sm text-muted-foreground">Expertise Areas</h3>
                    </div>
                    <div className="text-center p-4 rounded-lg bg-warning/5 hover:bg-warning/10 transition-colors">
                      <Badge className="bg-warning/10 text-warning">
                        {dataSupervisor.title}
                      </Badge>
                      <h3 className="text-sm text-muted-foreground mt-1">Title</h3>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="border hover:border-primary/50 transition-all p-0">
                  ...
                  <CardContent className="text-sm px-6 pb-6">
                    <div className="flex items-center gap-2 px-4 py-2 rounded-lg border-2 border-border w-fit">
                      ...
                      <span className="text-sm font-medium">{dataSupervisor.department?.name}</span>
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            {/* Supervised Projects Tab */}
            <TabsContent value="projects">
              <div className="space-y-4">
                {/* لازم تجيبي الـ projects اللي هو supervisor فيها من /api/projects/ وتفلتري */}
                <p className="text-sm text-muted-foreground text-center py-12">
                  No supervised projects to display yet.
                </p>
              </div>
            </TabsContent>
            ...
            <TabsContent value="expertise">
              <Card className="border hover:border-primary/50 transition-all">
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    <span className="text-md font-normal">Areas of Expertise</span>
                    <Button className="border bg-background text-foreground hover:bg-accent hover:text-accent-foreground dark:bg-input/30 dark:border-input dark:hover:bg-input/50">
                      <Plus className="size-4" />
                      Add Expertise
                    </Button>
      ...
      {role==="student" && <StudentProfile dataStudent={dataStudent}/>}
    </div>
  );
}